...
const AgingReport = () => {
  ...
  const getInstallmentRowStatus = (inst) => {
    ...
    if (monthsDiff < 0) return { key: 'unpaid', label: 'Unpaid' };
  ...
  const fetchAgingAccounts = async (branch, role) => {
    ...
    try {
      ...
      for (const [accId, list] of grouped) {
        ...
        const dueUnpaid = sortedInstallments.filter(p =>
        ...
        dueUnpaid.forEach(inst => {
          const overdueMonths = monthsBetween(inst.month, currentMonthStr) + 1;

          if (overdueMonths < 4) return;
    ...
    <div className="aging-container">
      <div className="aging-header">
        <div className="header-left">
          ...
          <p className="subtitle">Installments that are individually 4+ months overdue</p>
        </div>
        <ExportButton
          data={exportData}
      ...
      <div className="table-container">
        <div className="table-header">
          ...
          <span className="aging-info" style={{ fontWeight: 600 }}>Showing installments 4+ months overdue</span>
        </div>
